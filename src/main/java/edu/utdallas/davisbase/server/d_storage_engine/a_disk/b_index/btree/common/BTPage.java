package edu.utdallas.davisbase.server.d_storage_engine.a_disk.b_index.btree.common;
...
public class BTPage {
     ...
     */
    public BTPage(Transaction tx, BlockId currentblk, RecordValueLayout layout) {
        this.tx = tx;
    ...
    public int findSlotBefore(D_Constant searchkey) {
        int slot = 0;
        while (slot < getNumRecs() && getDataVal(slot).compareTo(searchkey) < 0) slot++;
        return slot - 1;
    }
    ...
    private void makeDefaultRecord(BlockId blk, int pos) {
        for (String fldname : layout.schema().fields()) {
            ...
            if (layout.schema().type(fldname) == INTEGER) tx.setInt(blk, pos + offset, 0, false);
            else tx.setString(blk, pos + offset, "", false);
        }
    }
     ...
     */
    public void insertLeaf(int slot, D_Constant val, RecordKey rid) {
        insert(slot);
        setVal(slot, "dataval", val);
    ...
    private D_Constant getVal(int slot, String fldname) {
        ...
        if (type == INTEGER) return new D_Constant(getInt(slot, fldname));
        else return new D_Constant(getString(slot, fldname));
    }
